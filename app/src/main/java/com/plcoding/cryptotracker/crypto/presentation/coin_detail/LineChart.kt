...
fun LineChart(
    ...
    Canvas(
        ...
        val labelCountExcludingLastLabel = ((labelViewPortHeightPx/ (xLabelLineHeight + minLabelSpacingYPx))).toInt() //only whole numbers for label count

        //e.g in image: (2659 - 2583)/2 = 38{difference between 2 labels}
...
//        )
        ...
        yLabelTextLayoutResults.forEachIndexed { index, result ->
            ...
            // visibleDataPointsIndices = 5..20;  it = 5, 5-5 = 0; 6-5 = 1; 7-5 = 2....
            drawPoints = visibleDataPointsIndices.map{
               val x = viewPortLeftX + (it - visibleDataPointsIndices.first) * xLabelWidth + xLabelWidth / 2f
                val ratio = (dataPoints[it].y - minYValue) / (maxYValue - minYValue)  //Interval mapping: [minYValue; maxYValue] -> [0; 1]
            ...
            //2 connection points for each coordinate
